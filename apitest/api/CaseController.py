...
import app
from apitest.model.CaseModel import Case  # 请根据您的模型路径进行调整
from datetime import datetime

# 模块信息
from core.tools import login_required

module_name = "Case"  # 模块名称，根据实际情况修改
module_model = Case
...
def queryByPage():
    ...
    try:
        ...
        with app.app.app_context():
            filter_list = []
            suite_id = int(request.json["suite_id"])
            filter_list.append(module_model.suite_id == suite_id)
            ...
            results = []
            for case in cases:
                api_case_id = ApiCollection.query.filter_by(id=case.api_case_id).first()
                result = respModel().get_custom_attributes(api_case_id)
